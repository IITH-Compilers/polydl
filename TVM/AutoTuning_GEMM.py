...
def gemmtvm(bf1,bf2,factor,M,N,K):
...
# Autotuning GEMM    
data = pd.read_csv("AutoTuningGemmData.csv",sep="[,]"  , header=None,engine='python')
for i in range(len(data)): 
    print("value",data.iloc[i,0],data.iloc[i,1],data.iloc[i,2],data.iloc[i,3],data.iloc[i,4],data.iloc[i,5])
    gemmtvm(int(data.iloc[i,0]),int(data.iloc[i,1]),int(data.iloc[i,2]),int(data.iloc[i,3]),int(data.iloc[i,4]),int(data.iloc[i,5]))
